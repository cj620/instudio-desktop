...
- [ ] 10.4 Validate packaged macOS, Windows, and Linux resources and smoke local package install, Webview, Agent tool, headless tool, and custom provider flows.
  ...
  - Native Linux PR, stable-release, and daily-prerelease jobs now fail closed after `linux-unpacked` backend/Chromium checks by directly launching the unique final x86_64 AppImage with `APPIMAGE_EXTRACT_AND_RUN=1`, no external `app.asar`, and a bounded Xvfb desktop smoke; upload remains ordered after that step. The resolver rejects zero/multiple, directory, symlink, wrong-architecture, and non-executable artifacts, and the public release gate enforces this wiring. This is executable CI coverage, not local native evidence until a commit-bound Linux x64 runner result and artifact are recorded.
  - Remaining release evidence: run the mandatory smoke with the signed macOS application and native Windows/Linux packages on their release runners, record the direct AppImage result on native Linux, and attach commit-bound logs/artifact hashes and reviewer sign-off. Workflow ordering and failure propagation are already enforced by the release gate.
