...
The missing capability is the product contract joining those pieces. A generic `delegate_task` result only contains a text summary, while normal tool blocks are folded into the work-process disclosure. Neither gives the renderer a durable, typed component prototype path or a reason to surface an interactive result in the answer body.

## Goals / Non-Goals

**Goals:**

- Give the main agent one purpose-built tool for designing or revising a single UI component interaction.
- Always execute the design work through a dedicated child-agent profile, not inside the renderer or main agent loop.
...
### 1. Add a specialized top-level tool over the existing delegation runtime

`design_component` is registered only in the main tool catalog and calls `DelegationRuntime.runChild` with the built-in `component-designer` profile. The child registry does not include the wrapper, preventing recursive component-design delegation. The profile inherits the parent's model/provider and approval policy but has an exact file-oriented tool allow-list with no shell or nested delegation. Its filesystem sandbox is deliberately narrowed to `workspace-write` rooted at the reserved artifact directory, so `write`/`edit` cannot touch product source even when the parent has full access.

The wrapper is preferable to teaching the model to assemble a fragile `delegate_task` prompt because it owns output-path reservation, bounded context packaging, progress events, artifact verification, and the structured result the GUI needs.

### 2. Use a versioned durable tool-result contract

The tool emits a `componentPrototype` payload with version, status, title, relative path, viewport, child id/profile, byte size, content hash, and summary. Preparing/running partial results use the same shape. Final tool history therefore rehydrates without a new HTTP endpoint or renderer-local registry.
...
Artifacts live at `.kun-design/component-prototypes/<slug>-<id>/prototype.html`. The wrapper creates a lightweight generating document before starting the child so the inline card can appear immediately. Existing code can be supplied directly or read from up to a bounded number of workspace-contained source files.

The child must produce a complete single-file document containing `meta[name="kun-component-prototype"]` and one `data-kun-component-root` root. The document body may center the component for demonstration, but may not add full-page navigation or application chrome.
...
- **The child writes malformed or page-level HTML.** The prompt and marker contract steer scope; structural/offline validation fails closed and returns an actionable tool error.
...
- **A running card points at a file that does not exist yet.** The wrapper reserves a generating document before emitting progress, and the existing preview host retries/watches revisions.
