...
- `smoke:packaged-extension-appimage` 只在原生 Linux x64 runner 执行。它要求 `dist` 中恰好一个规范命名的 x86_64 AppImage，拒绝目录、symlink 和 stale 多产物，补齐并复验 executable bit，然后以 `APPIMAGE_EXTRACT_AND_RUN=1` 走最终 AppImage 的 extraction/AppRun 路径。CLI fixture 仍由同次构建的 `linux-unpacked` runtime 安装，但 desktop executable 必须是 AppImage 本体，且不得追加外部 `app.asar`；因此它不是对 unpacked desktop smoke 的重复别名。该模式避免 CI 依赖 FUSE mount，但不证明 FUSE 或发行版安装器行为。
