...
- `smoke:packaged-extension-appimage` runs only on a native Linux x64 runner. It requires exactly one canonically named x86_64 AppImage in `dist`, rejects directories, symlinks, and multiple stale artifacts, sets and rechecks executable bits, then uses `APPIMAGE_EXTRACT_AND_RUN=1` to exercise the final AppImage extraction/AppRun path. The CLI fixture is still installed through the matching `linux-unpacked` runtime, but the desktop executable must be the AppImage itself with no external `app.asar`; this is not an alias for the unpacked desktop smoke. This avoids a FUSE-mount dependency in CI but does not prove FUSE or distribution installer behavior.
