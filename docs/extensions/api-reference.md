...
三个 SDK 当前版本均为 `1.0.0`，对应 Extension API major 1。Manifest 字段以生成的 [JSON Schema](../../packages/extension-api/schema/kun-extension.schema.json) 和 `ExtensionManifestSchema` 为机器真源；Host API、事件和 payload 以发布包的 `.d.ts` 与 runtime Schema 为真源。
...
| 包 | 用途 | 唯一受支持入口 |
| --- | --- | --- |
| `@kun/extension-api` | Framework-neutral Manifest、生命周期、Host client、Agent、工具、Provider、账号、存储、网络和 UI 契约 | `@kun/extension-api`；另有只读 `@kun/extension-api/manifest.schema.json` |
...
`createExtensionTestHarness`/`ExtensionTestHarness` 组合 `FakeHostTransport` 与 fake storage、workspace、Agent、tool、Provider、account、Webview service。测试仍应声明与生产相同的 permission/workspace/account scope，并覆盖 malformed payload、cancellation、disposal 和权限拒绝；fake service 不会让生产 Broker 自动放行。
