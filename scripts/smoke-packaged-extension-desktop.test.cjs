...
test('every automated and local release path gates uploads behind packaged Extension smokes', () => {
  ...
  const verifyMacX64Command =
    'npm run verify:packaged-macos-native -- --resources dist/mac-x64-verified/小元.app/Contents/Resources --arch x64'
  const smokeMacX64ExtensionsCommand =
    'npm run smoke:packaged-extensions -- --resources dist/mac-x64-verified/小元.app/Contents/Resources'
  const smokeMacX64DesktopCommand =
    'npm run smoke:packaged-extension-desktop -- --resources dist/mac-x64-verified/小元.app/Contents/Resources'

  assertPublishDependencies(release, 'stable release', ['build-macos', 'verify-macos-x64', 'build-windows'])
  assertPublishDependencies(daily, 'daily prerelease')

  assertOrderedCommands(release.jobs['build-macos'], [
    'npm run verify:packaged-macos-native -- --resources dist/mac/小元.app/Contents/Resources --arch x64',
    'npm run verify:packaged-macos-native -- --resources dist/mac-arm64/小元.app/Contents/Resources --arch arm64',
    packagedOcrCommand,
