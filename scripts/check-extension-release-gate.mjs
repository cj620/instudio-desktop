...
const releaseMacProductName = rootPackage.name === 'xiaoyuan-desktop' ? '小元' : 'Kun'
const verifyMacX64Command =
  `npm run verify:packaged-macos-native -- --resources dist/mac-x64-verified/${releaseMacProductName}.app/Contents/Resources --arch x64`
const smokeMacX64ExtensionsCommand =
  `npm run smoke:packaged-extensions -- --resources dist/mac-x64-verified/${releaseMacProductName}.app/Contents/Resources`
const smokeMacX64DesktopCommand =
  `npm run smoke:packaged-extension-desktop -- --resources dist/mac-x64-verified/${releaseMacProductName}.app/Contents/Resources`
...
const stableReleaseDependencies = isXiaoyuanStableRelease
  ? ['build-macos', 'verify-macos-x64', 'build-windows']
...
for (const [label, source] of [
  ...
  const requiredPlatformCount = label === 'Release' && isXiaoyuanStableRelease ? 2 : 3
  check(
    (source.match(/npm run smoke:extension-native-media/g) ?? []).length >= requiredPlatformCount &&
      (source.match(/npm run smoke:packaged-video-editor-native/g) ?? []).length >= requiredPlatformCount &&
      (source.match(/KUN_RUN_MEDIA_SMOKE: '1'/g) ?? []).length >= requiredPlatformCount,
  ...
  check(
    (label === 'Release' && isXiaoyuanStableRelease) ||
      (source.includes(videoEditorPackCommand) && source.includes('dist/kun-video-editor-*.kunx')),
...
requireOrderedCommands(releaseMacJob, 'build-macos', [
  ...
  stableMacBuildCommand,
  `npm run verify:packaged-macos-native -- --resources dist/mac/${stableMacProductName}.app/Contents/Resources --arch x64`,
  `npm run verify:packaged-macos-native -- --resources dist/mac-arm64/${stableMacProductName}.app/Contents/Resources --arch arm64`,
...
requireJobDependencies(
  releaseMacX64Job,
  'verify-macos-x64',
  isXiaoyuanStableRelease ? ['build-macos'] : ['prepare', 'build-macos']
)
