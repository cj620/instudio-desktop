export const SDD_RELATIVE_DIR = '.xiaoyuansdd'
/**
 * One requirement = one self-contained directory:
 * `.xiaoyuansdd/requirements/<uuid>/{requirement.md, trace.json, img/, proto/, chat/}`.
 * Plans stay under `.xiaoyuansdd/plan/sdd-<uuid>.md`, linked by the uuid.
 */
...
export function sddDraftFolderFromRelativePath(value: string): string | null {
  const normalized = normalizeSddRelativePath(value)
  const parts = normalized.split('/')
  if (parts.length !== 4 || parts[0] !== '.xiaoyuansdd' || parts[1] !== 'requirements') return null
  return UUID_LIKE.test(parts[2] ?? '') ? parts[2] : null
}

/** The requirement's self-contained unit directory (`.xiaoyuansdd/requirements/<uuid>`). */
...
/**
 * Map an SDD-generated plan path (`.xiaoyuansdd/plan/sdd-<uuid>[-n].md`) back to
...
export function sddDraftRelativePathForPlanPath(planRelativePath: string): string | null {
  ...
  const match = /^\.xiaoyuansdd\/plan\/sdd-([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})(?:-\d+)?\.md$/i.exec(
    normalized
  )
...
function isSddUnitSubPath(value: string, subdir: 'img' | 'proto'): boolean {
  ...
  if (parts[0] !== '.xiaoyuansdd' || parts[1] !== 'requirements') return false
