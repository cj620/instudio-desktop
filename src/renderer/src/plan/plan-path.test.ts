...
describe('plan-path', () => {
  it('keeps readable Chinese feature names', () => {
    ...
    expect(buildPlanRelativePath('做一个登录页')).toBe('.xiaoyuansdd/plan/做一个登录页.md')
  })
  ...
  it('falls back for empty or unsafe names', () => {
    ...
    expect(buildPlanRelativePath('../')).toBe('.xiaoyuansdd/plan/plan.md')
  })

  it('selects the next available duplicate path', () => {
    expect(
      nextAvailablePlanRelativePath('login', [
        '.xiaoyuansdd/plan/login.md',
        '.xiaoyuansdd/plan/login-2.md'
      ])
    ).toBe('.xiaoyuansdd/plan/login-3.md')
  })

  it('accepts only direct markdown files inside the GUI plan directory', () => {
    expect(isGuiPlanRelativePath('.xiaoyuansdd/plan/login.md')).toBe(true)
    expect(isGuiPlanRelativePath('.deepseekgui/plan/login.md')).toBe(true)
    expect(isGuiPlanRelativePath('.xiaoyuansdd/plan/nested/login.md')).toBe(false)
    expect(isGuiPlanRelativePath('../.xiaoyuansdd/plan/login.md')).toBe(false)
    expect(isGuiPlanRelativePath('.xiaoyuansdd/plan/login.txt')).toBe(false)
  })
})
