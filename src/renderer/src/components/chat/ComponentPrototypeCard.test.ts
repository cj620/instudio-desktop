...
describe('ComponentPrototypeCard metadata helpers', () => {
  ...
  it('clamps desktop/mobile frames and builds follow-up prompts around the artifact', () => {
    ...
    expect(componentPrototypeFrameSize(metadata, 'mobile')).toEqual({ width: 390, height: 520 })
...
describe('ComponentPrototypeCard interactions', () => {
  ...
  it('renders inline, switches viewport, refreshes, and keeps the preview in a private partition', async () => {
    expect(renderer.root.findByProps({ 'data-component-prototype-id': metadata.artifactId })).toBeTruthy()
    ...
    const mobile = renderer.root.findByProps({ title: 'Mobile' })
    await act(async () => mobile.props.onClick())
    expect(renderer.root.findAll((node) => node.props.style?.width === 390)).toHaveLength(1)

    const beforeRefresh = renderer.root.findByProps({ 'data-preview-host': true })
    await act(async () => renderer.root.findByProps({ 'aria-label': 'Refresh prototype' }).props.onClick())
  ...
  it('copies/views code and prefills the existing composer for iterate/adopt', async () => {
    await act(async () => {
      renderer.root.findByProps({ 'aria-label': 'Copy prototype code' }).props.onClick()
    ...
    const viewCode = buttons.find((button) => instanceText(button).includes('View code'))!
    const iterate = buttons.find((button) => instanceText(button).includes('Continue adjusting'))!
    const adopt = buttons.find((button) => instanceText(button).includes('Adopt design'))!
    await act(async () => viewCode.props.onClick())
    expect(actionMocks.previewWorkspaceFile).toHaveBeenCalledWith({
      path: metadata.relativePath,
      workspaceRoot: '/workspace'
    })
    await act(async () => iterate.props.onClick())
    await act(async () => adopt.props.onClick())
  ...
  it('shows a bounded failure state and disables follow-up actions', async () => {
    ...
    const followUps = renderer.root.findAllByType('button').filter((button) => (
      instanceText(button).includes('Continue adjusting') || instanceText(button).includes('Adopt design')
    ))
