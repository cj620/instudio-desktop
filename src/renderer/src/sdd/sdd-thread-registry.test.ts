...
function draft(partial: Partial<SddDraft> = {}): SddDraft {
  return {
    id: '/tmp/app:.xiaoyuansdd/draft/draft-1/requirement.md',
    workspaceRoot: '/tmp/app',
    relativePath: '.xiaoyuansdd/draft/draft-1/requirement.md',
...
describe('sdd-thread-registry', () => {
  ...
  it('releases the Requirement AI conversation into the Code sidebar after build starts', () => {
    ...
    expect(isSddAssistantThread({
      id: 'thread-sdd-1',
      title: '下一步: .xiaoyuansdd/draft/draft-1/requirement.md'
    }, registry)).toBe(false)
  })
  ...
  it('recognizes legacy SDD threads by draft paths even without registry data', () => {
    ...
    expect(isSddAssistantThread({
      ...
      workspace: '/tmp/app/.xiaoyuansdd/draft/draft-1'
    }, registry)).toBe(true)
    expect(isSddAssistantThread({
      id: 'thread-unit-layout',
      title: '下一步: .xiaoyuansdd/requirements/123e4567-e89b-12d3-a456-426614174000/requirement.md'
    }, registry)).toBe(true)
    expect(isSddAssistantThread({
      id: 'thread-normal',
      title: '需求 AI',
