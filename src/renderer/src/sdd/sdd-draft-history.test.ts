...
function directory(entries: Array<{ name: string, type: 'file' | 'directory', path?: string }>): WorkspaceDirectoryListResult {
  return {
    ok: true,
    root: '/tmp/app/.xiaoyuansdd/requirements',
    entries: entries.map((entry) => ({
      name: entry.name,
      path: entry.path ?? `/tmp/app/.xiaoyuansdd/requirements/${entry.name}`,
      type: entry.type,
...
describe('sdd-draft-history', () => {
  ...
  it('discovers disk-only requirement drafts under .xiaoyuansdd/requirements', async () => {
    ...
    const listWorkspaceDirectory = vi.fn(async (options: WorkspaceDirectoryTarget) => {
      if (options.path === '.xiaoyuansdd/requirements') {
        return directory([
      ...
      if (options.path === `.xiaoyuansdd/requirements/${draftId}`) {
        return directory([
          {
            name: 'requirement.md',
            type: 'file',
            path: `/tmp/app/.xiaoyuansdd/requirements/${draftId}/requirement.md`
          }
        ])
      }
    ...
    expect(history[0]).toMatchObject({
      ...
      relativePath: `.xiaoyuansdd/requirements/${draftId}/requirement.md`,
