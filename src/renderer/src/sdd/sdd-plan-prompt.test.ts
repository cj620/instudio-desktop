...
function image(partial: Partial<SddDraftImageReference> = {}): SddDraftImageReference {
  return {
    ...
    relativePath: '.xiaoyuansdd/img/wireframe.png',
    mimeType: 'image/png',
...
describe('buildSddDraftToPlanPrompt', () => {
  it('keeps Markdown image syntax and maps visual attachments by image number', () => {
    const prompt = buildSddDraftToPlanPrompt({
      ...
      draftRelativePath: '.xiaoyuansdd/draft/draft-1/requirement.md',
      planRelativePath: '.xiaoyuansdd/plan/sdd-draft-1.md',
    ...
    expect(prompt).toContain('Reserved plan file: .xiaoyuansdd/plan/sdd-draft-1.md')
    expect(prompt).toContain('`plan_relative_path` to `.xiaoyuansdd/plan/sdd-draft-1.md`')
  })
