...
// 对话会话不绑定项目文件夹,默认在 ~/Documents/Xiaoyuan(macOS/Windows)或
// ~/.local/share/Xiaoyuan/conversations(Linux)下按时间戳创建工作目录。
export function defaultConversationWorkspaceRoot(): string {
  ...
  return platform === 'linux' ? '~/.local/share/Xiaoyuan/conversations' : '~/Documents/Xiaoyuan'
}
// 兼容旧引用;动态取值。
...
export function isInternalDeepSeekGuiWorkspace(path?: string): boolean {
  ...
  return (
    ...
    || normalized === '~/.xiaoyuan/design-workspace'
    || normalized.endsWith('/.xiaoyuan/design-workspace')
