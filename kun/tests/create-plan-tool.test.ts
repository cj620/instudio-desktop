...
function buildGuiPlan(relativePath = '.xiaoyuansdd/plan/login.md', operation: 'draft' | 'refine' = 'draft') {
  return {
    operation,
...
describe('create_plan tool: path validation', () => {
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# nested',
        operation: 'draft',
        plan_relative_path: '.xiaoyuansdd/plan/nested/a.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.xiaoyuansdd/plan/nested/a.md')
      })
    )
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# not md',
        operation: 'draft',
        plan_relative_path: '.xiaoyuansdd/plan/login.txt'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.xiaoyuansdd/plan/login.txt')
      })
    )
    expect(result.isError).toBe(true)
  })
    ...
    const result = await executeCreatePlanTool(
      {
        markdown: '# mismatch',
        operation: 'refine',
        plan_relative_path: '.xiaoyuansdd/plan/other.md'
      },
      buildContext({
        threadMode: 'plan',
        guiPlan: {
          operation: 'refine',
          workspaceRoot: '/tmp/ws',
          relativePath: '.xiaoyuansdd/plan/login.md',
          planId: 'plan_login'
        }
      })
    )
  ...
  it('rejects an operation that differs from the active GUI plan operation', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        threadMode: 'plan',
        guiPlan: buildGuiPlan('.xiaoyuansdd/plan/login.md', 'refine')
      })
    )
    expect(result.isError).toBe(true)
...
describe('create_plan tool: execution safety', () => {
  it('allows a free-form plan-mode call without a GUI plan context and self-allocates a path', async () => {
    ...
    expect((result.output as { relative_path: string }).relative_path).toBe(
      '.xiaoyuansdd/plan/disk-cleanup.md'
    )
  })
  ...
  it('rejects a free-form legacy plan path as a new target', async () => {
    ...
    expect(result.isError).toBe(true)
    expect(JSON.stringify(result.output)).toMatch(/\.xiaoyuansdd\/plan/)
  })
  ...
  it('still executes create_plan through the tool host during plan mode', async () => {
    ...
    expect(result.item.kind === 'tool_result'
      ...
      : ''
    ).toBe('.xiaoyuansdd/plan/safe-plan.md')
  })
})
...
describe('create_plan tool: success and atomic write', () => {
  ...
  beforeEach(async () => {
    ...
    await mkdir(join(workspace, '.xiaoyuansdd/plan'), { recursive: true })
    await writeFile(join(workspace, '.xiaoyuansdd/plan/login.md'), previousMarkdown, 'utf8')
  })
  ...
  it('writes a fresh plan to the reserved path and returns structured metadata', async () => {
    const result = await executeCreatePlanTool(
      ...
      buildContext({
        ...
        guiPlan: {
          ...
          relativePath: '.xiaoyuansdd/plan/login.md',
          planId: `${workspace}:.xiaoyuansdd/plan/login.md`,
    ...
    expect(output.relative_path).toBe('.xiaoyuansdd/plan/login.md')
    expect(output.operation).toBe('draft')
    expect(output.summary).toContain('.xiaoyuansdd/plan/login.md')
  ...
  it('self-allocates a non-colliding path when plan mode has no reserved context', async () => {
    ...
    // `.xiaoyuansdd/plan/login.md` already exists in this workspace.
    expect(output.relative_path).toBe('.xiaoyuansdd/plan/login-2.md')
    const persisted = await readFile(output.absolute_path, 'utf8')
  ...
  it('leaves the previous plan untouched when the abort signal fires before rename', async () => {
    ...
    expect(result.isError).toBe(true)
    const persisted = await readFile(join(workspace, '.xiaoyuansdd/plan/login.md'), 'utf8')
