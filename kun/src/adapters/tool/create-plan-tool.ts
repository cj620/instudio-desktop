...
const TOOL_DESCRIPTION = [
  ...
  '.xiaoyuansdd/plan and returns structured metadata. Call again to revise.'
].join(' ')
...
export const CREATE_PLAN_INPUT_SCHEMA: Record<string, unknown> = {
  ...
  properties: {
    ...
    plan_relative_path: {
      type: 'string',
      description: 'Optional reserved relative path; must live directly under .xiaoyuansdd/plan.'
    }
  },
...
export type CreatePlanAdapterOptions = {
  ...
  /**
   * Lists existing plan relative paths (e.g. `.xiaoyuansdd/plan/foo.md`)
 ...
 *    `.xiaoyuansdd/plan/<feature>.md` under the active workspace.
...
function resolveReservedTarget(
  ...
  if (!relativePath || !isGuiPlanRelativePath(relativePath)) {
    return { error: 'plan_relative_path must be a direct Markdown file under .xiaoyuansdd/plan' }
  }
 ...
 * allocates a fresh, non-colliding `.xiaoyuansdd/plan/<feature>.md`.
 */
